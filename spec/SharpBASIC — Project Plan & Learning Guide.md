...
You are building **SharpBASIC** — a modern BASIC-inspired programming language with no
line numbers, implemented as a tree-walking interpreter in C# .NET 10. It is a
fixed-scope learning project that ends at Phase 10. The project serves three goals
simultaneously:
...
Each stage is a separate C# project with its own test project. Stages communicate
only through well-defined types — no coupling between non-adjacent stages.

---

## Scope Declaration

SharpBASIC has a fixed, defined scope. It is a learning project with a clear end point.
The language is done when Phase 10 is complete. No features are to be added beyond
what is specified in the 10 phases. When SharpBASIC is finished, a new project begins.

**Explicitly out of scope for SharpBASIC:**
...
> **Scope is fixed.** SharpBASIC ends at Phase 10. Language-level File I/O and
> compile-to-executable are explicitly out of scope — they belong to the next language
> project. Do not add them here.

| Phase | Content | Honest Difficulty |
|---|---|---|
| 1 | Lexer, tokens, Hello World | Easy |
| 2 | Parser, AST, recursive descent | Moderate |
| 3 | Variables, symbol table, assignment | Moderate |
| 4 | Expressions, Pratt parsing, precedence | **Hard** |
| 5 | IF / THEN / ELSE / END IF | Moderate |
| 6 | FOR / NEXT, WHILE / WEND | Moderate |
...
### Phase 1 — Lexer Foundations & Hello World *(Start here)*
...
- `Repl` — reads a line, lexes it, evaluates PRINT
...
public void Lex_PrintStatement_ReturnsCorrectSequence()
{
    ...
    var tokens = lexer.Tokenise()
        .Where(t => t.Type != TokenType.Whitespace).ToList();
...
### Phase 4 — Expressions & Arithmetic ⚠️ *Expect to slow down here*
...
Assign each operator a binding power (precedence). `*` and `/` bind tighter than
`+` and `-`. The parser recurses based on these powers — elegant and extensible.
This is the first significant difficulty spike. Take the time to understand it
fully before moving on. Crafting Interpreters Chapter 6 covers this in detail.
...
### Phase 7 — Subroutines & Functions ⚠️ *Expect to slow down here*
...
Managing scope and the call stack correctly is the second major difficulty spike.
The test suite written here will pay dividends repeatedly.
...
**Goal:** Errors are clear, located (line/col), and structured throughout the pipeline.
...
- `Result<T>` return types hardened across all stages
...
**What you'll build:**
- Built-in functions: `LEN`, `MID$`, `LEFT$`, `RIGHT$`, `UPPER$`, `LOWER$`,
  `INT`, `STR$`, `VAL`, `ABS`, `SQR`, `RND`
...
SharpBASIC is complete at the end of this phase.
...
public sealed class Lexer
{
    ...
    private Token ConsumeIdentifierOrKeyword()
    {
        ...
        var type = Keywords.TryGetValue(text.TrimEnd('$'), out var kw)
            ? kw
...
### `SharpBasic.Lexer.Tests` — Phase 1 Test Suite
...
while (true)
{
    ...
    try
    {
        ...
        if (tokens.Count >= 2
            ...
            && tokens[1].Type == TokenType.StringLiteral)
        {
...
Then move to Phase 2 — build the Parser so PRINT is properly parsed into an AST
rather than handled ad-hoc in the REPL.

---

## End of SharpBASIC

SharpBASIC is complete at the end of Phase 10. It is a deliberate, fixed-scope learning
project. Features not listed in the 10 phases — including language-level File I/O,
compile-to-executable, a bytecode VM, and a VS Code extension — are explicitly out of
scope. They are not stretch goals. They belong to the next project.

---

## What Comes Next — Language 2

After SharpBASIC is finished, a second language project begins. Before starting it,
write a short retrospective covering:

- What was harder than expected and why
- What architectural decisions you would change
- What the test suite caught that manual testing would have missed
- What you would design differently in the AST

That retrospective becomes the foundation of the Language 2 specification.

### Language 2 — Deferred Ideas

These features came up during SharpBASIC planning and were explicitly deferred.
They are candidates for Language 2, not SharpBASIC additions.
...
| Module / import system | `import "utils"` to split programs across files |
| VS Code extension | TextMate grammar for highlighting, LSP for errors |

Language 2 is not designed until SharpBASIC Phase 10 is complete and the
...
| Resource | Notes |
|---|---|
| Crafting Interpreters — Robert Nystrom | Primary reference. Physical copy recommended. Chapters 1–4 already read. |
| craftinginterpreters.com | Free web version — useful for searching |
| Let's Build a Simple Interpreter — Ruslan Spivak | ruslanspivak.com — good for Phase 1–3 |
| The Super Tiny Compiler — James Kyle | github.com/jamiebuilds/the-super-tiny-compiler |
| Computerphile — Parsing videos | YouTube — good visual intuition for the pipeline |
| Roslyn source — github.com/dotnet/roslyn | Inspiration for error handling and AST design |

---

*Single definitive project plan. No supplementary files required.*
*Update the First Run Checklist as phases complete.*
