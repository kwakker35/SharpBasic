...
|March 2026|SEARCH while monster alive — reveals presence, combat begins unconditionally      |Issuing SEARCH in a room with a live fixed monster triggers immediate combat. The searchInterrupt flag is passed to CombatLoop — SKILL -1 on round 1, additive with any existing Lesser Terror penalty. A specific high-flavour line prints before combat begins (one per fixed monster, room-grounded random lines for the zombie). Applies to all rooms with live monsters including the wandering zombie. Exception: Bound King (room 11) — SEARCH refused outright, no combat triggered, no penalty.|
...
|March 2026|Underscore support added to identifiers                                           |Underscore permitted between word characters in identifiers. SCREAMING_SNAKE_CASE for constants (CONST MAX_WIDTH, CONST ITEM_GOLD_BAG), snake_case for variables if desired. Leading underscore, trailing underscore, consecutive underscores, and bare _ are all invalid. Primary use case is multi-word CONST names.|
|March 2026|Paging model — 30-row terminal, 20 content rows                                  |Game targets 30-row Windows terminal. Chrome (separators, header, location, exits, prompt) occupies 10 rows, leaving 20 content rows. CONST SCREEN_HEIGHT=30, CONST CONTENT_ROWS=20. No automatic line counting — pauses are authored at [PAUSE] markers in the content asset file. SUB Pause() prints 'Press ENTER to continue.' and waits.|
|March 2026|Win sequence requires one authored pause                                          |Win sequence is 29 lines, exceeding the 20 content row limit. CALL Pause() inserted after 'The guards don't move.' and before the quoted speech begins. This is the only text block in the game requiring a pause — all others fit within 20 lines.|
|March 2026|SEARCH interrupt — specific flavour lines per monster                             |Each fixed monster has one specific high-flavour interrupt line that fires when SEARCH triggers combat. Lines convey the monster's character — the Brute reacts to sound, the Horror was never unaware, the Guardian turns without hurry, the Mage knew before you moved, the Troll finds you with patient eyes. Zombie has 3 random room-grounded lines per room, drawn from physical objects present in that room. All text locked in content asset file Deliverable 8.|
|March 2026|Sneak formula confirmed — RollDice(2) <= skill                                   |Flat SKILL check per design log spec. No per-monster modifier. Roll 2d6 — success if result <= current SKILL. ~58% at SKILL 7, ~72% at SKILL 9, ~97% at SKILL 12. Mirrors TestLuck pattern for consistency.|
|March 2026|Room 6 impossible state removed — monster alive with passage found               |SEARCH while the Skittering Horror is alive triggers immediate combat. The passage can therefore never be found while the Horror lives. The 'monster alive, passage found' room description variant has been removed. Two valid monster-alive states exist: first visit and revisit.|
|March 2026|Riddle Room revisit text corrected — only correct door open, wrong door sealed    |Original text showed both doors open on revisit. Corrected: the door the player chose stands open, the other is sealed with no handle, no gap. 'Whatever is on the other side stays there.' Players who died there on a prior run will understand.|
|March 2026|Combat tension lines added — player and monster thresholds                        |Four lines fire inside CombatLoop based on STAMINA thresholds, once per fight each. Player <=4: 'You are barely standing. So is your chance of walking out of here.' Player <=2: 'You are one blow from the end. You both know it.' Monster <=4: 'You can feel the momentum shifting. It is yours to lose now.' Monster <=2: 'It is nearly done. Don't give it a way back.'|
|March 2026|Crushing Blow flavour line added — Bound King only                               |When Bound King wins a round by 3+ attack strength: 'The blow lands with the weight of three hundred years of experience. It takes you to one knee and you feel something break inside.'|
|March 2026|General low STAMINA line added — fires on room entry                             |When player STAMINA <= 4, fires before room description on room entry: 'You have paid dearly just to be standing here. You drive your battered body on.'|
|March 2026|Bound King — third fight impossible, gold mechanic confirmed                      |After second fight (triggered by regeneration), gold loop does not run again. No third regeneration check. goldBags persists across Throne Room visits — thresholds always reflect total bags taken this run. Regeneration is bag-driven, not turn-driven.|
