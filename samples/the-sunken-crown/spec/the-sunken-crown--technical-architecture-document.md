...
(or useful) when the monster is still alive. The mechanic is defined only for
post-combat scenarios. Recommended: allow the command but have the monster
block it — “The Brute watches your every move. You can’t search with it standing there.”

-----

## Appendix: Outstanding Ambiguities

These are unresolved questions found in the design log. Do not implement these
as assumptions without designer confirmation.

|#|Location          |Ambiguity                                                                                                                                                                     |
|-|------------------|------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
|1|ADR-007           |“Return south from [5] reaches [2]” vs the luck-roll pool which does not include [2]. Are these the same mechanic or different?                                               |
|2|ADR-010 / Combat  |Exact interaction between Sword of Sharpness and the Armoured mechanic — does the sword negate armour, reduce it, or add +1 to player damage?                                 |
|3|Room State Machine|Can the player SEARCH room [2] while the Brute is alive?                                                                                                                      |
|4|ADR-004           |Does poison damage fire before or after the room description?                                                                                                                 |
|5|State Variable Map|Item storage — integer item codes recommended but not specified in the log                                                                                                    |
|6|Combat            |Hollow Mage luck drain when luck = 0 — silently floored or flagged?                                                                                                           |
|7|Loot              |Design log mentions 5 loot slots but also lists 6 in the slot assignment table (slots 1–6, with slot 6 in the Underhall). The shuffled pool is 4 items. Confirm: 5 or 6 slots?|

**On ambiguity #7:** The Loot Slot Assignments table in the design log lists 6 slots
(rows 1–6, with slot 6 being the Underhall). The Loot Philosophy section says “4 items
across 5 loot slots.” These are inconsistent. The table is the more detailed and later
entry — 6 slots is likely correct, with the “5 slots” being an earlier draft.
**Confirm with designer.**

-----

*Document produced from design log. Nothing invented. Ambiguities preserved.*
*Update this document when design log decisions resolve any noted ambiguity.*